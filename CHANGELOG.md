...
### Added
- New `bw_gui.widgets.ScrollableFrame`: a real `ttk.Frame` subclass that is itself a scrollable viewport (vertical Canvas+Scrollbar, `.content` as the build target, no `__getattr__` delegation needed since it already is the widget) - the shared SSOT for embedded, non-popup scrolling (see new `docs/SCROLLABILITY_CONTRACT.md`). Mousewheel is dispatched through exactly one `bind_all` per Tk interpreter (not per instance), so any number of simultaneously visible `ScrollableFrame`s - including ones nested deep inside `PanedWindow` panes - never clobber each other's binding, and a wheel event over any child widget inside `.content` still resolves to the right instance.
